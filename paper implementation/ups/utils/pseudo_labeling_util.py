# TODO 이쪽 tqdm(dataloader)는 따로빼기 for uncertainty
...
def pseudo_labeling(args, data_loader, model, itr):
    ...
    if not args.no_progress_pseudo:
        data_loader = tqdm(data_loader)

    with torch.no_grad():
        ## TODO 
        filenames = list()
        uncertainties_output = list()
        uncertainties_u = list()
        preds = list()
        ...
        for batch_idx, (inputs, targets, indexs, _) in enumerate(data_loader):
            ...
            ## 이 부분에서 ambiguity vaule (msp, mc-dropout값) 저장 
            ## + 선택된 값은 어떤 값인지 확인
            #print('==> check batch idx : ', batch_idx)
            filename, _ = data_loader.dataset.samples[batch_idx]
            filenames.append(filename)
            uncertainties_output.append(max_value)
            uncertainties_u.append(max_std.squeeze(1))
            ...
            print('==> check selected idx :', selected_idx) # selected idx갯수가 적으면, threshold가 너무 큰 값이라는 생각이 듦
            ...
            prec1 = accuracy(outputs[selected_idx], targets[selected_idx], topk=(1, ))

            losses.update(loss.item(), inputs.shape[0])
            prec1.to(args.device)
            top1.update(prec1, inputs.shape[0])
#             top5.update(prec5.item(), inputs.shape[0])
            batch_time.update(time.time() - end)
            end = time.time()
            if not args.no_progress_pseudo:
                data_loader.set_description("Pseudo-Labeling Iter: {batch:4}/{iter:4}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. top1: {top1:.2f}.  ".format(
                    batch=batch_idx + 1,
...
#         certainty_output = sns.kdeplot(uncertainties_output, shade = True)
#         certainty_u = sns.kdeplot(uncertainties_u, shade = True)
#         certainty_output.savefig("msp.png")
#         certainty_u.savefig('mcdropout.png')
        
#         filenames = np.asarray(filenames)
#         uncertainties_output = np.asarray(uncertainties_output)
#         uncertainties_u = np.asarray(uncertainties_u)
#         preds = np.asarray(preds)
        ...
        #df = pd.DataFrame({
        #    'filenames':filenames,
        #    'uncertainties_output':uncertainties_output,
        #    'uncertainties_u':uncertainties_u,
        #    'preds':preds
        #})
        tmp_idx = time.time()
        #df.to_csv(f'outputs_uncertainity/uncertainties_{tmp_idx}.csv')
        ##
    ...
    print('>> check pseudo targets : ', pseudo_target)
    print('>> check gt_target  :', gt_target)
    print('>> check pseudo maxstd :', pseudo_maxstd)
    print('>> check pseudo idx :',  pseudo_idx)
    ...
    if itr < args.class_blnc-1:
        ...
        print('==> check num_classes : ', args.num_classes)
        ...
        if len(blnc_idx_list) == 0 :
            ...
            print('==> check blnc_idx_list : ', blnc_idx_list)
    ...
    ## TODO 
    print('==> check nl_gt_list :', nl_gt_list)
    ...
    pseudo_label_dict = {'filenames' : filenames, 'pseudo_idx': pseudo_idx.tolist(), 'pseudo_target':pseudo_target.tolist(), 'nl_idx': pseudo_nl_idx, 'nl_mask': pseudo_nl_mask.tolist()}
