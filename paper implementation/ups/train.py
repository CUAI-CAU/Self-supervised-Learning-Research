...
from data.cifar import get_imagenet_person, get_imagenet
...
def main():
    ...
    parser.add_argument('--dataset', default='imagenet_all', type=str,
                        choices=['cifar10', 'cifar100',  'imagenet_person'],
                        help='dataset names')
    ...
    parser.add_argument('--iterations', default=10, type=int,################################# TODO
                        help='number of total pseudo-labeling iterations to run')
    parser.add_argument('--epchs', default=100, type=int, ################################# TODO
    ...
    parser.add_argument('--no-progress-pseudo', default = True, action = 'store_true')
    ...
    ## TODO 
    DATASET_GETTERS = {'imagenet_person': get_imagenet_person, 'imagenet_all' : get_imagenet_person}
    ...
    device = 'cuda:1' #torch.device('cuda', args.gpu_id)
    args.device = device
    ...
    elif args.dataset == 'imagenet_person' :
        args.num_classes = 2
    ...
    for itr in range(start_itr, args.iterations):
        ...
        ## TODO 
        lbl_dataset, nl_dataset, unlbl_dataset, test_dataset = DATASET_GETTERS[args.dataset]()

        model = create_model(args)
        model.to(args.device)
        
        ## TODO : 여기 smaples 안읽히는것 어떡할까
        ...
        #TODO 
#         lbl_loader = train_dataloader
#         nl_loader = train_dataloader
#         test_loader = test_dataloader
#         unlbl_loader = train_dataloader
        ...
        nl_loader = DataLoader(
            nl_dataset,
            sampler=RandomSampler(nl_dataset),#dtypes = np.array,
        ...
        test_loader = DataLoader(
            test_dataset,
            sampler=SequentialSampler(test_dataset),#dtypes = np.array, 
            batch_size=args.batch_size,
            num_workers=args.num_workers)
        
        unlbl_loader = DataLoader(
            unlbl_dataset,
            sampler=SequentialSampler(unlbl_dataset),#dtypes = np.array, 
            batch_size=args.batch_size,
            num_workers=args.num_workers)
        ...
        for epoch in range(start_epoch, args.epochs):
            ## TODO: iter=0, else 인 경우 모두 수정 필요
            if itr == 0:
        ...
        # pseudo label로 분류된 값 저장하는 부분
        with open(os.path.join(args.out, f'pseudo_labeling_iteration_{str(itr+1)}.pkl'),"wb") as f:
            pickle.dump(pseudo_label_dict,f)

        print(f'PL Acc (Positive): {pl_acc_pos}, Total Selected (Positive): {total_sel_pos}\n')
        print(f'PL Acc (Negative): {pl_acc_neg}, Total Selected (Negative): {total_sel_neg}, Unique Negative Samples: {unique_sel_neg}\n\n')
